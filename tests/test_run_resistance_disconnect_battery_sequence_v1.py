...
class PublicInputTests(unittest.TestCase):
    ...
    def test_source_video_must_stay_under_configured_root(self) -> None:
        with self.assertRaisesRegex(ValueError, "source_video_must_be_relative"):
            runner.discover_video(str(Path(ROOT.anchor) / "private" / "sample.mp4"), ROOT)
