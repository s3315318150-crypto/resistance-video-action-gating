...
| v2 时序保护废案（仅保留历史审计，入口已禁用） | `scripts/qwen_experiment_action_hierarchical_v2_temporal_guard.py`、`scripts/qwen_hierarchical_v2_temporal_guard_reduce.py` |
| v2 时序保护 + Rubric 边界精修 | `scripts/refine_v2_temporal_guard_boundaries_rubric.py`，详见 `docs/v2_temporal_guard_plus_rubric_boundary_readme.md` |
| v2 行为容错系列 | `scripts/qwen_experiment_action_hierarchical_v2_behavior_tolerant*.py`，详见 `docs/v2_behavior_tolerant_series_readme.md` |
...
`scripts/run_all_rubrics_v2.py` 是证据工件的汇总器。它读取当前 run 已生成的动作分割结果与专项证据工件，并把最终评分统一为 `pass` 或 `fail`；它不读取 Excel 标签，也不修改原始视频。新视频的正式取证优先由 Agent 调用动态 Skill 完成。

`run_all_rubrics_v2.py`、`run_meter_record_consistency_v1.py` 和带有按视频 ROI 配置的 Rubric 8 入口可以用于显式的离线 replay/regression；它们不是 Agent 的 fresh `execute` 路径。fresh 执行不得加载历史时间窗、固定视频 ROI、历史预测或 Excel 标签。
...
先运行各评分项的证据提取器，将其输出路径填入匿名配置，再运行总评（适用于已有工件的离线汇总）：
...
默认动作分割使用原始 v2 的 Map、Reduce、七阶段赋值和提示词，输入最长边为 1280px。要试验新版 v3，显式增加：
...
要在原始 v2 上增加按当前视频缺失测量位置触发的自适应补帧 Agent，使用：

```powershell
python scripts/run_resistance_pipeline.py `
  --video-dir data/videos `
  --output-root outputs/resistance_pipeline `
  --action-version v2-frame-agent
```

`v2-screenshot-guard` 和 `v2-screenshot-guard-agent` 只保留用于历史回归，不属于正式默认路径。

已有 Temporal Guard v2 分段后，可以追加 Rubric 引导的 0.5 秒边界精修。该步骤不修改原阶段数量、顺序或时间段，只输出独立的 `refined_boundaries`：
...
算法说明见 [v2 Temporal Guard + Rubric 边界精修](docs/v2_temporal_guard_plus_rubric_boundary_readme.md)，Yes/No 对照实验见 [Yes/No 边界实验记录](docs/v2_temporal_guard_yes_no_boundary_experiment.md)。

行为容错系列是四个逐项累积的独立选项，旧入口均保持不变：

| `--action-version` | 增量能力 |
|---|---|
| `v2-behavior-tolerant` | Temporal Guard + 带惩罚行为状态图 |
| `v2-behavior-tolerant-aux` | 再增加不竞争的辅助动作 |
| `v2-behavior-tolerant-boundary` | 再增加窗口边缘局部复核 |
| `v2-behavior-tolerant-adaptive` | 再增加最多 25% 的补充式动态帧 |
...
脚本会从 `circuit_rewiring` 生成 episode，也会在重复 `recording_1` 或 `measurement_1` 之间搜索短暂间隔，避免遗漏短换线。所有端点和开关证据必须留在同一个 episode 内，不能跨时间拼接。该入口保留用于离线回归；Agent fresh execute 使用当前 run 的动态取证器。
