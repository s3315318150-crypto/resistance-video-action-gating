...
segments, seven-stage actions by default, and a freshly generated wiring episode config.
...
ACTION_VARIANTS = {
    ...
    "five-stage": (
        "qwen_experiment_action_hierarchical_v2_five_stage.py",
        "resistance_5stage_measurement_recording_v1.json",
    ),
    "v2-behavior-tolerant": (
        "qwen_experiment_action_hierarchical_v2_behavior_tolerant.py",
        "resistance_7stage_no_battery_v2.json",
    ),
    "v2-behavior-tolerant-aux": (
        "qwen_experiment_action_hierarchical_v2_behavior_tolerant_aux.py",
        "resistance_7stage_no_battery_v2_behavior_tolerant_aux.json",
    ),
    "v2-behavior-tolerant-boundary": (
        "qwen_experiment_action_hierarchical_v2_behavior_tolerant_boundary.py",
        "resistance_7stage_no_battery_v2_behavior_tolerant_aux.json",
    ),
    "v2-behavior-tolerant-adaptive": (
        "qwen_experiment_action_hierarchical_v2_behavior_tolerant_adaptive.py",
        "resistance_7stage_no_battery_v2_behavior_tolerant_aux.json",
    ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--action-max-model-edge", type=int, default=1280)
    parser.add_argument("--action-jpeg-quality", type=int, default=90)
    parser.add_argument(
        "--action-version",
        choices=tuple(ACTION_VARIANTS),
        default="v2",
        help=(
            "v2 is the active original-Reduce 1280px seven-stage path; "
            "v2-frame-agent adds bounded adaptive frame requests without Screenshot Guard. "
            "Screenshot Guard variants remain explicit regression-only choices. "
            "Behavior-tolerant variants and v3 remain explicit opt-ins."
        ),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not 40 <= args.action_jpeg_quality <= 100:
        raise ValueError("action_jpeg_quality_must_be_between_40_and_100")
