...
def build_map_prompt(
    ...
    return f"""你是一名严谨的中学物理实验视频判读员。当前输入是一个匿名的“伏安法测电阻”实验视频。
...
def build_reduce_prompt(video_id: str, events: list[dict[str, Any]]) -> str:
    ...
    return f"""你是分层视频分析算法的全局 Reduce 审核员。当前输入是一个匿名的“伏安法测电阻”实验视频。
...
def build_boundary_prompt(
    ...
    merged_measurement_recording = (
        "measurement_1" not in stage_labels
        and "measurement_2" not in stage_labels
        and {"recording_1", "recording_2"}.intersection(stage_labels)
    )
    stage_evidence_rule = (
        "五阶段模式中 `recording_1` / `recording_2` 表示同一轮的测量记录区间；"
        "观察电表、读取示数或直接书写都可作为该合并阶段的起止证据，不能强行把两种重叠动作切成两个阶段。"
        if merged_measurement_recording
        else "第二次测量和第二次记录仍必须有指定阶段的直接可见动作证据。"
    )
    return f"""你是匿名“伏安法测电阻”视频的动作边界复核员。现在只复核边界 `{boundary['boundary_id']}`：从 `{from_stage}`（{stage_labels[from_stage]}）转为 `{to_stage}`（{stage_labels[to_stage]}）。
...
5. 不要根据实验标准流程补造动作。{stage_evidence_rule}
