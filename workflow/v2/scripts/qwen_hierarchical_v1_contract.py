...
def load_stage_schema(path: Path) -> dict[str, Any]:
    ...
    for item in stages:
        accepted = item.get("accepted_base_actions")
        if accepted is not None:
            if (
                not isinstance(accepted, list)
                or not accepted
                or any(action not in BASE_ACTIONS[:-1] for action in accepted)
            ):
                raise ValueError("stage_accepted_base_actions_invalid")
            continue
        if item.get("required_base_action") not in BASE_ACTIONS[:-1]:
