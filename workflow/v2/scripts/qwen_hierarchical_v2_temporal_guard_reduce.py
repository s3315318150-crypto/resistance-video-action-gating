...
def salvage_reduce_response_with_temporal_guard(
    ...
    # The archived wrapper used to own this recovery.  The current base
    # reducer performs the same generic repair earlier in the pipeline and
    # records it as ``nonoverlapping_conflict_rejection_recovered``.  Keep the
    # old temporal_guard contract populated from that current-run repair log,
    # but only for events that survive the remainder of base reduction (for
    # example, a post-terminal event must not be reported as restored).
    base_restored_ids = {
        str(repair["event_id"])
        for repair in repairs
        if isinstance(repair, dict)
        and repair.get("reason") == "nonoverlapping_conflict_rejection_recovered"
        and isinstance(repair.get("event_id"), str)
        and str(repair["event_id"]) in accepted_ids
    }
    restored_ids: list[str] = [
        str(event["event_id"])
        for event in events
        if str(event["event_id"]) in base_restored_ids
    ]
    for event_id in restored_ids:
        # Preserve the repair reason consumed by archived replay tests and
        # reports while retaining the base reducer's canonical repair record.
        repairs.append(
            {
                "reason": "non_overlapping_rejection_restored",
                "event_id": event_id,
                "original_rejection_reason": "conflicts_with_stronger_evidence",
                "restored_by": "base_reduce_compatibility_mapping",
            }
        )
