...
DIRECT_EXPERIMENT_ACTIONS = {
    "wiring_action",
    "measurement_action",
    "writing_action",
}
RESUMED_EXPERIMENT_MIN_CONFIDENCE = 0.8
RESUMED_EXPERIMENT_MIN_SPAN_SECONDS = 20.0
...
def resumed_experiment_events(
    events: list[dict[str, Any]],
    cleanup_event: dict[str, Any],
) -> list[dict[str, Any]]:
    """Return strong current-run evidence that the experiment continued.

    A single incidental action after cleanup remains post-experiment noise. A
    sustained sequence containing at least two direct action types over twenty
    seconds means the cleanup candidate was an intermediate move, not a terminal
    barrier.
    """
    cleanup_end_frame = int(cleanup_event["last_frame_number"])
    candidates = [
        event
        for event in events
        if event.get("action_type") in DIRECT_EXPERIMENT_ACTIONS
        and int(event["first_frame_number"]) > cleanup_end_frame
        and float(event.get("confidence") or 0.0) >= RESUMED_EXPERIMENT_MIN_CONFIDENCE
    ]
    if len(candidates) < 2 or len({str(item["action_type"]) for item in candidates}) < 2:
        return []
    first_seconds = min(float(item["first_seconds"]) for item in candidates)
    last_seconds = max(float(item["last_seconds"]) for item in candidates)
    if last_seconds - first_seconds < RESUMED_EXPERIMENT_MIN_SPAN_SECONDS:
        return []
    return sorted(candidates, key=lambda item: int(item["first_frame_number"]))
...
def salvage_reduce_response(
    ...
    # Reduce only sees Map metadata, so it may hallucinate that sequential
    # events overlap. A non-overlapping direct action cannot be removed as a
    # conflict; preserve the current-run Map evidence and let the deterministic
    # state machine decide its stage.
    for event_id, rejection in list(rejected_by_id.items()):
        event = event_by_id[event_id]
        if (
            rejection["reason"] != "conflicts_with_stronger_evidence"
            or event.get("action_type") not in DIRECT_EXPERIMENT_ACTIONS
        ):
            continue
        overlapping_accepted_ids = [
            accepted_id
            for accepted_id in accepted
            if _overlaps(event, event_by_id[accepted_id])
        ]
        if overlapping_accepted_ids:
            continue
        accepted.append(event_id)
        del rejected_by_id[event_id]
        repairs.append(
            {
                "reason": "nonoverlapping_conflict_rejection_recovered",
                "event_id": event_id,
                "accepted_event_ids_checked": list(accepted[:-1]),
            }
        )
    ...
    if terminal_id is not None:
        ...
        resumed_events = resumed_experiment_events(events, terminal)
        if resumed_events:
            resumed_ids = [str(event["event_id"]) for event in resumed_events]
            accepted = [event_id for event_id in accepted if event_id != terminal_id]
            rejected_by_id[terminal_id] = {
                "event_id": terminal_id,
                "reason": "other",
                "explanation": "后续持续出现多类实验动作，本地将该整理候选判为中途移动。",
            }
            recovered_ids: list[str] = []
            for event in resumed_events:
                event_id = str(event["event_id"])
                rejection = rejected_by_id.get(event_id)
                if rejection is None or rejection.get("reason") != "post_terminal_cleanup":
                    continue
            ...
            repairs.append(
                {
                    "reason": "terminal_cleanup_demoted_by_resumed_experiment",
                    "event_id": terminal_id,
                    "resumed_event_ids": resumed_ids,
                    "recovered_event_ids": recovered_ids,
                    "resumed_action_types": sorted(
                        {str(event["action_type"]) for event in resumed_events}
                    ),
                    "resumed_span_seconds": round(
                        max(float(event["last_seconds"]) for event in resumed_events)
                        - min(float(event["first_seconds"]) for event in resumed_events),
                        3,
                    ),
                }
            )
...
def assign_five_stages(
    accepted_events: list[dict[str, Any]],
    terminal_cleanup_event_id: str | None,
) -> dict[str, Any]:
    """Assign five stages while retaining measurement and writing sub-actions.

    `recording_1` and `recording_2` are compatibility IDs for merged
    measurement-recording cycles. Their labels and diagnostic fields make the
    merged meaning explicit; downstream consumers can keep using the existing
    recording stage IDs without losing measurement evidence.
    """
    five_stages = (
        "circuit_wiring",
        "recording_1",
    ...
    for event in events:
        ...
        item["stage"] = None
        item["assignment_reason"] = "unclassified_action"
        item["merged_stage"] = False
        item["base_action_type"] = action
        if terminal_reached:
            discarded_after_terminal.append(event_id)
            continue
        if action == "cleanup_action":
        ...
        if action == "wiring_action":
            if phase == 0:
                item["stage"] = "circuit_wiring"
                item["assignment_reason"] = "wiring_before_first_measurement_recording_cycle"
            elif phase == 1:
                item["stage"] = "circuit_rewiring"
                item["assignment_reason"] = "wiring_after_first_measurement_recording_cycle"
                phase = 2
            elif phase == 2:
                item["stage"] = "circuit_rewiring"
                item["assignment_reason"] = "continued_rewiring_before_second_cycle"
            else:
                item["assignment_reason"] = "wiring_would_create_unsupported_third_cycle"
        elif action in {"measurement_action", "writing_action"}:
            item["merged_stage"] = True
            item["merged_stage_semantics"] = "measurement_and_recording_cycle"
            if phase in (0, 1):
                item["stage"] = "recording_1"
                item["assignment_reason"] = (
                    "first_cycle_measurement_evidence"
                    if action == "measurement_action"
                    else "first_cycle_writing_evidence"
                )
                phase = 1
            elif phase in (2, 3):
                item["stage"] = "recording_2"
                item["assignment_reason"] = (
                    "second_cycle_measurement_evidence"
                    if action == "measurement_action"
                    else "second_cycle_writing_evidence"
                )
                phase = 3
        if item["stage"] is None:
            review_reasons.append(f"unclassified_transition:{event_id}:{item['assignment_reason']}")
    ...
    observed_intervals = [
        {
            ...
            "confidence": item["confidence"],
            "assignment_reason": item["assignment_reason"],
            "base_action_type": item["base_action_type"],
            "merged_stage": bool(item.get("merged_stage")),
            "merged_stage_semantics": item.get("merged_stage_semantics"),
        }
        for item in assigned
        if item.get("stage") is not None
    ]
    observed_stages = {str(item["stage"]) for item in observed_intervals}
    return {
        "stage_model": "five_stage_measurement_recording_merge",
        "assigned_events": assigned,
        "observed_stage_intervals": observed_intervals,
        "missing_stages": [stage for stage in five_stages if stage not in observed_stages],
        "review_reasons": sorted(set(review_reasons)),
        "analysis_termination": {
            "terminal_cleanup_reached": terminal_reached,
            "terminal_cleanup_event_id": terminal_cleanup_event_id if terminal_reached else None,
...
def merge_observed_stage_runs(
    ...
    for index, run in enumerate(runs, start=1):
        ...
        merged = run["stage"] in {"recording_1", "recording_2"} and any(
            item.get("action_type") in {"measurement_action", "writing_action"}
            for item in run["observed_subintervals"]
        )
        if merged:
            run["stage_semantics"] = "measurement_and_recording_cycle"
            run["cycle_index"] = int(str(run["stage"]).rsplit("_", 1)[1])
            ...
            run["merged_measurement_recording"] = True
            run["stage_window_semantics"] = "measurement_and_recording_cycle"
            run["merged_stage"] = True
            run["merged_stage_semantics"] = "measurement_and_recording_cycle"
    return runs
