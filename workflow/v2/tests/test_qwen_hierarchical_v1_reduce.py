...
class HierarchicalReduceTests(unittest.TestCase):
    ...
    def test_sustained_experiment_resumption_demotes_false_cleanup(self) -> None:
        candidates = [
            event("e1", "wiring_action", 0, 30),
            {
                **event("e2", "cleanup_action", 60, 90, 0.95),
                "evidence": "学生把部分器材移动到左上角。",
            },
            event("e3", "writing_action", 150, 300, 0.95),
            event("e4", "measurement_action", 900, 960, 0.9),
            event("e5", "wiring_action", 1500, 1560, 0.95),
        ]
        response = {
            ...
            "rejected_events": [
                {
                    ...
                    "explanation": "误认为实验已经结束。",
                }
                for event_id in ("e3", "e4", "e5")
            ],
            "conflicts": [],
            "terminal_cleanup_event_id": "e2",
            "confidence": 0.9,
        ...
        self.assertEqual(
            ["e3", "e4", "e5"],
            [item["event_id"] for item in resumed_experiment_events(candidates, candidates[1])],
        )
        repaired, repairs = salvage_reduce_response(candidates, response)

        self.assertIsNotNone(repaired)
        assert repaired is not None
        self.assertIsNone(repaired["terminal_cleanup_event_id"])
        self.assertEqual({"e1", "e3", "e4", "e5"}, set(repaired["accepted_event_ids"]))
        self.assertEqual(["e2"], [item["event_id"] for item in repaired["rejected_events"]])
        self.assertEqual([], repaired["ignored_noise_events"])
        repair = next(
            item
            for item in repairs
            if item["reason"] == "terminal_cleanup_demoted_by_resumed_experiment"
        )
        self.assertEqual(["e3", "e4", "e5"], repair["recovered_event_ids"])
        self.assertEqual(
            ["measurement_action", "wiring_action", "writing_action"],
            repair["resumed_action_types"],
        )
    ...
    def test_nonoverlapping_reduce_conflict_is_recovered_for_two_cycles(self) -> None:
        candidates = [
            event("e1", "writing_action", 100, 114),
            event("e2", "wiring_action", 116, 126),
            event("e3", "writing_action", 128, 138),
        ]
        response = {
            "accepted_event_ids": ["e1", "e3"],
            "rejected_events": [
                {
                    "event_id": "e2",
                    "reason": "conflicts_with_stronger_evidence",
                    "explanation": "错误地声称中间接线与前后书写重叠。",
                }
            ],
            "conflicts": [
                {
                    "event_ids": ["e1", "e2", "e3"],
                    "resolution": "错误地声称三个事件重叠。",
                }
            ],
            "terminal_cleanup_event_id": None,
            "confidence": 0.9,
        ...
        self.assertIsNotNone(repaired)
        assert repaired is not None
        self.assertEqual({"e1", "e2", "e3"}, set(repaired["accepted_event_ids"]))
        self.assertEqual([], repaired["rejected_events"])
        self.assertTrue(
            any(
                item["reason"] == "nonoverlapping_conflict_rejection_recovered"
                and item["event_id"] == "e2"
                for item in repairs
            )
        )
        assigned = assign_five_stages(
            [
                next(item for item in candidates if item["event_id"] == event_id)
                for event_id in repaired["accepted_event_ids"]
            ],
            None,
        )
        self.assertEqual(
            ["recording_1", "circuit_rewiring", "recording_2"],
            [item["stage"] for item in assigned["observed_stage_intervals"]],
        )

    def test_overlapping_reduce_conflict_remains_rejected(self) -> None:
        candidates = [
            event("e1", "writing_action", 100, 130),
            event("e2", "wiring_action", 116, 126),
        ]
        response = {
            "accepted_event_ids": ["e1"],
            "rejected_events": [
                {
                    "event_id": "e2",
                    "reason": "conflicts_with_stronger_evidence",
                    "explanation": "接线与书写证据实际重叠。",
                }
            ],
            "conflicts": [
                {"event_ids": ["e1", "e2"], "resolution": "保留书写事件。"}
            ],
            "terminal_cleanup_event_id": None,
            "confidence": 0.9,
            "uncertainty": "",
        ...
        self.assertEqual(["e2"], [item["event_id"] for item in repaired["rejected_events"]])
        self.assertFalse(
            any(item["reason"] == "nonoverlapping_conflict_rejection_recovered" for item in repairs)
        )
