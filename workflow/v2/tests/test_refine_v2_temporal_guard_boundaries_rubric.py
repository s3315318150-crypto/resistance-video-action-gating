...
class RubricBoundaryEntrypointTests(unittest.TestCase):
    ...
    def test_plain_v2_result_is_accepted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            result = root / "result.json"
            result.write_text(
    ...
    def test_five_stage_result_is_accepted_and_uses_merged_labels(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            result = root / "result.json"
            result.write_text(
            ...
            records = MODULE.load_action_records(summary)
            labels = MODULE.stage_labels_for_records(records)
            self.assertEqual("第一次测量记录", labels["recording_1"])
            self.assertNotIn("measurement_1", labels)
    ...
    def test_v2_schema_loads_after_explicit_binding(self) -> None:
        MODULE.support.bind_mature_v2_pipeline()
        schema = MODULE.support.contract.load_stage_schema(
            MODULE.support.v2_entrypoint.DEFAULT_SCHEMA
        )
