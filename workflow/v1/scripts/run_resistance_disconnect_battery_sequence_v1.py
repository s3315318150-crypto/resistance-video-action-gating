...
DEFAULT_OUTPUT_ROOT = ROOT / "outputs" / "resistance_disconnect_battery_sequence_v1"
DEFAULT_ROI_CONFIG = ROOT / "configs" / "resistance_disconnect_battery_rois.example.json"
...
def video_id_from_name(value: str) -> str:
    ...
    return match.group(1) if match else safe_slug(stem)
...
def discover_video(source_name: str, video_root: Path) -> Path:
    ...
    if relative.is_absolute():
        raise ValueError(f"source_video_must_be_relative:{source_name}")
    source = (root / relative).resolve()
    if not source.is_relative_to(root):
        raise ValueError(f"source_video_outside_video_root:{source_name}")
    if not source.is_file():
        raise FileNotFoundError(f"source_video_missing:{source}")
    return source
...
def episode_candidates(intervals: Mapping[str, list[list[float]]]) -> list[dict[str, Any]]:
    ...
    """
    supplied = [list(value) for value in intervals.get("circuit_rewiring", [])]
    ...
    accepted_intervals = list(supplied)
    for stage in RECOVERY_STAGE_IDS:
        stage_values = sorted((list(value) for value in intervals.get(stage, [])), key=lambda value: (value[0], value[1]))
...
def load_roi_map(path: Path) -> dict[str, tuple[float, float, float, float]]:
    if not path.is_file():
        return {}
    document = read_json(path)
    result: dict[str, tuple[float, float, float, float]] = {}
    for item in document.get("samples", []) if isinstance(document, Mapping) else []:
        if not isinstance(item, Mapping):
            continue
        raw_id = str(item.get("video_id") or "")
        try:
            video_id = video_id_from_name(raw_id)
        except ValueError:
            continue
        roi = item.get("model_rois", {}).get("source_terminal_detail")
        if isinstance(roi, list) and len(roi) == 4:
            values = tuple(float(v) for v in roi)
            if 0 <= values[0] < values[2] <= 1 and 0 <= values[1] < values[3] <= 1:
                result[video_id] = values  # type: ignore[assignment]
    return result


def load_reference_map(path: Path) -> dict[str, Path]:
    if not path.is_file():
        return {}
    document = read_json(path)
    ...
    for item in document.get("samples", []) if isinstance(document, Mapping) else []:
        ...
        if reference.is_file():
            result[video_id] = reference.resolve()
    return result
...
class BatteryRoiTracker:
    """Track the reference battery region through camera motion."""

    def __init__(self, reference_path: Path | None, fallback_roi: tuple[float, float, float, float] | None, initial_roi: tuple[float, float, float, float] | None = None) -> None:
        self.fallback_roi = expand_roi(fallback_roi) if fallback_roi else None
        self.reference = None
        self.reference_keypoints = None
        self.reference_descriptors = None
        self.reference_size: tuple[int, int] | None = None
        self.reference_gray_small = None
        self.last_roi = initial_roi or self.fallback_roi
        self.previous_keypoints = None
        self.previous_descriptors = None
        self.orb = cv2.ORB_create(nfeatures=1800)
        if reference_path is None or not reference_path.is_file() or self.fallback_roi is None:
            return
        image = cv2.imread(str(reference_path))
        if image is None:
            return
        image = resize_longest(image, 960)
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        self.reference_gray_small = cv2.resize(gray, (480, 270), interpolation=cv2.INTER_AREA)
        ...
        if descriptors is not None and len(keypoints) >= 20:
            ...
            self.reference_size = (image.shape[1], image.shape[0])
    ...
    def locate(self, frame: np.ndarray) -> tuple[tuple[float, float, float, float] | None, str]:
        ...
        mode = "last_valid" if self.last_roi else "fallback"
...
def extract_frames(
    ...
    sample_fps: float,
    output_dir: Path,
    battery_roi: tuple[float, float, float, float] | None,
    ...
    tracker = BatteryRoiTracker(reference_path, battery_roi, initial_roi)
    target_index = 0
    current = targets[0]
    try:
        ...
        while target_index < len(targets):
            ...
            if current == targets[target_index]:
                ...
                fallback_battery = crop_normalized(frame, tracker.fallback_roi) if tracker.fallback_roi is not None else frame
                ...
                if not cv2.imwrite(str(fallback_path), banner(fallback_battery, f"FRAME ID={frame_id} | VIDEO T={timestamp:.3f}s | REFERENCE ROI", 1200), [cv2.IMWRITE_JPEG_QUALITY, 95]):
                    raise RuntimeError(f"Could not write {fallback_path}")
                records.append(FrameRecord(frame_id, current, timestamp, str(panorama_path.resolve()), str(battery_path.resolve()), sharpness, motion, battery_motion, tracked_roi, roi_mode, str(fallback_path.resolve())))
...
def paired_image(record: FrameRecord, output_path: Path) -> Path:
    ...
    cv2.putText(canvas, "REFERENCE ROI", (panorama.shape[1] + battery.shape[1] + 8, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
...
def run_structured_summary(client: Any, model: str, records: list[FrameRecord], episode: Mapping[str, Any], output_dir: Path, use_cache: bool = True) -> tuple[dict[str, Any] | None, str | None, str]:
    ...
    if use_cache and response_path.is_file():
        try:
            ...
            if isinstance(cached_parsed, Mapping) and not validate_structured_summary(cached_parsed, selected, episode):
...
def create_client(base_url: str, token: str) -> Any:
    ...
    return OpenAI(base_url=base_url, api_key=token, timeout=180, max_retries=1)
...
def run_screening(client: Any, model: str, records: list[FrameRecord], episode: Mapping[str, Any], output_dir: Path, use_cache: bool = True) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    ...
    for index, chunk in enumerate(split_chunks(records, 12), start=1):
        ...
        make_contact_sheet(chunk, sheet, "paired")
        ...
        if use_cache and response_path.is_file():
            try:
                cached = read_json(response_path)
                parsed = cached.get("parsed") if isinstance(cached, Mapping) else None
                error = cached.get("error") if isinstance(cached, Mapping) else None
                raw = str(cached.get("raw") or "") if isinstance(cached, Mapping) else ""
            except (OSError, ValueError, json.JSONDecodeError):
                parsed = None
        ...
        reports.append({"chunk_index": index, "frame_ids": [r.frame_id for r in chunk], "status": "parsed", "battery_candidate_frame_ids": b, "switch_candidate_frame_ids": s, "cleanup_like_frame_ids": c, "notes": parsed.get("notes", [])})
    result = {"battery_candidate_frame_ids": sorted(set(battery_ids)), "switch_candidate_frame_ids": sorted(set(switch_ids)), "cleanup_like_frame_ids": sorted(set(cleanup_ids))}
...
def process_episode(
    ...
    start, end = episode["expanded_interval_seconds"]
    core_start, core_end = episode["core_interval_seconds"]
    coarse = extract_frames(video_path, episode["video_id"], episode["episode_id"], start, end, 2.0, output_dir / "coarse", roi, reference_path)
    write_json(output_dir / "coarse_manifest.json", {"frames": [item.as_dict() for item in coarse]})
    screening, screen_reports = run_screening(None if prepare_only else client, model, coarse, episode, output_dir / "screening") if coarse else (None, [])
    ...
    for index, (left, right) in enumerate(merged, start=1):
        initial = None
        if coarse:
            initial = min(coarse, key=lambda item: abs(item.timestamp_seconds - left)).battery_roi
        dense.extend(extract_frames(video_path, episode["video_id"], episode["episode_id"] + f"_d{index}", left, right, 10.0 if (left, right) != (core_start, core_end) else 5.0, output_dir / "dense" / f"range_{index:02d}", roi, reference_path, initial))
    ...
    if supplied_observations is not None:
        ...
        structured_summary, structured_error, structured_raw = run_structured_summary(client, model, dense_selected, episode, output_dir / "facts")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--video-ids", default="", help="Optional comma-separated source video IDs")
    ...
    roi_config = args.roi_config if args.roi_config.is_absolute() else ROOT / args.roi_config
    ...
    roi_map = load_roi_map(roi_config)
    reference_map = load_reference_map(roi_config)
    supplied = load_supplied_map(args.observations_root)
    if not args.prepare_only and not supplied and (not args.api_base_url or not args.api_token):
        raise RuntimeError("missing_qwen_configuration:QWEN_API_BASE_URL,QWEN_API_TOKEN")
    ...
    for source_record in action_document.get("records", []):
        ...
        source_video = discover_video(source_name, video_root)
        _, duration, _ = video_duration(source_video)
        episodes: list[dict[str, Any]] = []
        episode_indexes: dict[str, int] = {}
        for candidate in episode_candidates(intervals):
            ...
            episodes.append(process_episode(episode, source_video, roi_map.get(video_id), ep_dir, client, args.model, supplied_item, args.prepare_only, reference_map.get(video_id)))
        ...
        video_record = {
            ...
            "reason": "; ".join(str(item.get("reason")) for item in episodes if item.get("reason")) or "No circuit_rewiring episode was supplied.",
    ...
    summary = {
        ...
        "video_root": str(video_root.expanduser().resolve()),
        "roi_config": str(roi_config.resolve()),
        "reference_frames": {key: str(value) for key, value in reference_map.items()},
