...
class TemporalGuardTests(unittest.TestCase):
    ...
    def test_entrypoint_binds_new_identity_without_changing_original_file(self) -> None:
        entrypoint.bind_v2_temporal_guard()
        self.assertEqual("qwen_experiment_action_hierarchical_v2_temporal_guard", engine.ALGORITHM_ID)
        self.assertIs(engine.salvage_reduce_response, salvage_reduce_response_with_temporal_guard)
        self.assertIn("local_partial", entrypoint.normalized_argv([]))
