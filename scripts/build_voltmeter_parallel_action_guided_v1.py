...
def source_video_candidates(source_dir: Path) -> list[Path]:
    return sorted(
        ...
        if "phy-resistance" in path.name.casefold()
    )


def source_video_key(value: str) -> tuple[str | None, str | None]:
    prefix = re.match(r"^(\d+)(?:_|$)", Path(value).name)
    timestamp = re.search(r"(\d{14})", Path(value).name)
    return (prefix.group(1) if prefix else None, timestamp.group(1) if timestamp else None)


def find_source_video(source_video_id: str, candidates: list[Path]) -> tuple[Path, str]:
    exact = [path for path in candidates if path.name.casefold() == source_video_id.casefold()]
    if len(exact) == 1:
        return exact[0], "exact_source_video_id"
    prefix, timestamp = source_video_key(source_video_id)
    matched = []
    for path in candidates:
        candidate_prefix, candidate_timestamp = source_video_key(path.name)
        if prefix and timestamp and candidate_prefix == prefix and candidate_timestamp == timestamp:
            matched.append(path)
    if len(matched) == 1:
        return matched[0], "numeric_prefix_and_timestamp"
    details = [str(path) for path in (exact or matched or candidates)]
    raise ValueError(f"Could not resolve a unique resistance MP4 for {source_video_id}: {details}")
...
def main() -> int:
    ...
    if not source_videos:
        raise SystemExit(f"No top-level phy-resistance MP4 files found under {source_dir}")
