#!/usr/bin/env python3
"""Refine existing Temporal Guard v2 boundaries with rubric-guided frames.
...
normal Temporal Guard summary and the deterministic replay summary used by the
...
EXPERIMENT_SCRIPT = (
    ...
    / "scripts"
    / "v2_retrieval_boundary_only_ab_v3.py"
)
DEFAULT_PROFILES = (
    ROOT
    / "experiments"
...
def load_support() -> Any:
    if not EXPERIMENT_SCRIPT.is_file():
        raise FileNotFoundError(f"boundary_retrieval_support_missing:{EXPERIMENT_SCRIPT}")
    spec = importlib.util.spec_from_file_location("v2_rubric_boundary_support", EXPERIMENT_SCRIPT)
    if spec is None or spec.loader is None:
        raise ImportError(EXPERIMENT_SCRIPT)
...
def load_action_records(summary_path: Path) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        elif result.get("algorithm_id") != "qwen_experiment_action_hierarchical_v2_temporal_guard":
            raise ValueError(
                f"not_temporal_guard_v2:{record.get('source_video_id')}:{result.get('algorithm_id')}"
            )
...
def refine_one(
    ...
    prepared = support.base.prepare_shell(baseline, video_dir)
    ...
    for boundary in candidates:
        ...
        observed = support.base.engine._observed_boundary_from_pass(
            boundary_pass,
    ...
    refined, rejected = support.base.engine._enforce_boundary_monotonicity(refined)
    payload = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    support.base.bind_mature_v2_pipeline()
    schema = support.base.contract.load_stage_schema(support.base.v2_entrypoint.DEFAULT_SCHEMA)
    ...
    if not args.prepare_only:
        ...
        client = support.base.CountingClient(
            support.base.qwen_base.OpenAI(
                base_url=endpoint,
                api_key=token,
    ...
    engine_args = support.base.default_engine_args()
