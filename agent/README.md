...
- 每次从当前视频重新识别接线、测量、记录、改线和整理阶段；当前发布集为完整 `R0-R9`。
...
R7/R9 已接入正式 execute。Agent 分别绑定 `recording_1`、`recording_2` 的纸面 U/I 和同轮电表读数；只有原记录比较通过，并且当前 run 的 R4 正负接线柱、R5 指针正常偏转、R6 量程合适全部为 `pass` 时，R7/R9 才能 `pass`。任一前置项明确失败，记录项立即判 `fail`。Bundle 会自动先生成 R4/R5/R6，门控不读取历史预测、固定视频 ROI 或 Excel。
