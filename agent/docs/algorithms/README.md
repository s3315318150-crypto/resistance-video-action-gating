...
本文档是当前 Agent 八个 Rubric 的统一技术入口。每项都按同一结构说明：评分目标、使用阶段、抽帧方式、动态 ROI、OpenCV/Qwen 分工、本地融合、输出和限制。
...
| Rubric | 已记录开发集口径 | 限制 |
|---|---:|---|
| R0 | 未形成独立准确率 | 需要 fresh 冻结评测 |
| R1 | Agent 总口径 `4/5 = 80%` | 仍有误判，专项文档含较旧结果 |
| R2 | 映射后 `5/5`，明确二分类 `4/4` | 明确样本偏向 pass，fail recall 未充分验证 |
| R3 | `5/5 = 100%` | 属于开发集，不是泛化证明 |
...
当前 Agent 专项与契约测试共 `95` 项，最近一次本机执行全部通过。
